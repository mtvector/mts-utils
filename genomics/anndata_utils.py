...
def convert_to_dataframe(data):
    ...
    for key, value in data.items():
        if isinstance(value, dict) and 'categories' in value and 'codes' in value:
            categories = [cat.decode('utf-8') if isinstance(cat, bytes) else cat for cat in value['categories']]
            codes = value['codes']
            df_dict[key] = pd.Categorical.from_codes(codes, categories)
        elif isinstance(value, dict) and 'data' in value and 'indices' in value and 'indptr' in value:
            df_dict[key] = csr_matrix((value['data'], value['indices'], value['indptr']))
        else:
        ...
        try:
            for layer in data['layers']:
                data['layers'][layer] = csr_matrix(
                    (data['layers'][layer]['data'], data['layers'][layer]['indices'], data['layers'][layer]['indptr'])
                )
        except:
            pass
    return data
